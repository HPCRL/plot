...
def add_bars(ax, ansor_data, ansordynamic_data, index, bar_width):
    ...
    for i, row in ansor_data.iterrows():
        ...
        ax.bar(index[i] - bar_width, ansor_1_mean, bar_width, yerr=ansor_1_error, label='Ansor (1_mean)' if i == 0 else '', color='lightblue', hatch="//", capsize=5)
        ax.bar(index[i] - bar_width, ansor_2_mean - ansor_1_mean, bar_width, yerr=ansor_2_error, label='Ansor (2_mean)' if i == 0 else '', color='blue', hatch="\\\\", bottom=ansor_1_mean, capsize=5)
        
        # Plot ansordynamic bars with hatch patterns
        ax.bar(index[i], ansordynamic_1_mean, bar_width, yerr=ansordynamic_1_error, label='ansordynamic (1_mean)' if i == 0 else '', color='orange', hatch="xx", capsize=5)
        ax.bar(index[i], ansordynamic_2_mean - ansordynamic_1_mean, bar_width, yerr=ansordynamic_2_error, label='ansordynamic (2_mean)' if i == 0 else '', color='red', hatch="||", bottom=ansordynamic_1_mean, capsize=5)  # Use orange color for 2_mean
